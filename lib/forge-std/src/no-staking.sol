...
contract StakeSystem {
    ...
    */
    ...
    /* function stake(uint256 _amount, uint256 _duration) external { tuve que cambiar external a public para poder acceder desde test */
    ...
    */
    ...
    function unStake() external {
        bool rewardControl = rewardCalculated(msg.sender);//falta testear
        if (rewardControl) {//falta testear
            require(contractBalance > StakeTokensUsers[msg.sender].tokenReward, ContractNotBalance() ); // testeado
            totalTokensReward = StakeTokensUsers[msg.sender].amountTokenStaked + StakeTokensUsers[msg.sender].tokenReward;//falta testear
            contractBalance = contractBalance - StakeTokensUsers[msg.sender].tokenReward;//falta testear
            StakeTokensUsers[msg.sender].amountTokenStaked = 0;//falta testear
            emit unStakeSuccess("UnStake amount plus rewards", totalTokensReward);//falta testear
        } else {
            totalTokensReward = StakeTokensUsers[msg.sender].amountTokenStaked;//falta testear
            StakeTokensUsers[msg.sender].amountTokenStaked = 0;//falta testear
            emit NotRewardForTimestamp("UnstakeWithoutReward", totalTokensReward );//falta testear
        }
    ...
    */
    ...
    // Agregar esta función en el contrato principal
    function getStakeInfo(address _user) public view returns (uint256, uint256, uint256, uint256, bool) {
        UserStakingBody memory userStake = StakeTokensUsers[_user];
        return (
            userStake.amountTokenStaked,
            userStake.durationStake,
            userStake.blockTimestamp,
            userStake.tokenReward,
            userStake.exist
        );
    }
